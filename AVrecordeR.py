...
if __name__ == "__main__":
    ...
    parser.add_argument("--record_setting_path", default="setting/record_setting.json")
    ...
    path = os.path.join(dirname, filename)  # ex.) "Nara/01_2022_12_31/01_expert"
    path_video = path + video_file_name  # ex.) "Nara/01_2022_12_31/01_expert.raw.mp4"
    path_audio_main = (
    ...
    utils.file_manager(path_video)  # Delete when file already exists
    utils.file_manager(path_audio_main)  # Delete when file already exists
    utils.file_manager(path_audio_sub)  # Delete when file already exists
    utils.file_manager(path_log)  # Delete when file already exists
    ...
    audio_device_id_main = getDeviceID.get_audio_id(audio_device_main)
    audio_device_id_sub = getDeviceID.get_audio_id(audio_device_sub)
    logger.debug(f"Video Device: [{video_device_id}]{video_device}")
    logger.debug(f"Audio Device (MAIN): [{audio_device_id_main}]{audio_device_main}")
    logger.debug(f"Audio Device (SUB): [{audio_device_id_sub}]{audio_device_sub}")
    ...
    audio_thread_main = audioRecorder.AudioRecorder(
    ...
    audio_thread_sub = audioRecorder.AudioRecorder(
        path=path_audio_sub,
    ...
    video_thread = videoRecorder.VideoRecorder(
        path=path_video,
    ...
    logger.debug(f"Start time of audio_thread_mic: {audio_thread_main.start_time}")
    logger.debug(f"Start time of audio_thread_mic: {audio_thread_main.end_time}")
    logger.debug(f"Start time of audio_thread_kinect: {audio_thread_sub.start_time}")
    logger.debug(f"Start time of audio_thread_kinect: {audio_thread_sub.end_time}")

    # File Format
    logger.info("File Formatting...")
    fileFormatting.cut_to_same_length(
